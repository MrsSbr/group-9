...
public class FlowerShop {
    ...
    public int getMonthWithMostVersatileBouquets() {
        ...
        orders.entrySet()
                .forEach(order -> {
                    ...
                    order.getValue().forEach(fo -> months.get(curMonth).addBouquet(fo.getCompositionSize()));
                });
    ...
    public Map<String, Double> getTypeTotalSales() {
        ...
        orders.entrySet().stream()
                .filter(order -> order.getKey().getYear() == curYear)
                .forEach(order -> order.getValue()
                        .forEach(fo -> {
    ...
    public Map<String, DeliveryPickupCasesAmount> getFlowersWithDeliveryFrequency() {
        ...
        orders.entrySet().forEach(order -> order.getValue()
                .forEach(fo -> fo.getComposition()
                        .forEach(flower -> {
